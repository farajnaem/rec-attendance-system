...
<div class="grid-2">
    ...
    <a href="<?= e(url('/manager/departments')) ?>" class="card" style="text-decoration:none;color:inherit">
        <h3>الدوائر</h3>
        <p class="text-muted">إدارة الدوائر ومشرفيها</p>
    </a>
    <?php endif; ?>
    <?php if (RoleHelper::isOrgAdmin(Auth::role())): ?>
    <a href="<?= e(url('/manager/work-schedule')) ?>" class="card" style="text-decoration:none;color:inherit">
        <h3>ساعات الدوام</h3>
        <p class="text-muted">تحديد أوقات العمل الرسمية</p>
    </a>
    <?php endif; ?>
    <?php if (Auth::can('manage_locations')): ?>
    <a href="<?= e(url('/manager/locations')) ?>" class="card" style="text-decoration:none;color:inherit">
        <h3>مواقع العمل</h3>
        <p class="text-muted">إحداثيات GPS للحضور</p>
    </a>
    <?php endif; ?>
    <?php if (Auth::can('manage_job_description')): ?>
    <a href="<?= e(url('/manager/job-description')) ?>" class="card" style="text-decoration:none;color:inherit">
    ...
    <a href="<?= e(url('/manager/attendance')) ?>" class="card" style="text-decoration:none;color:inherit">
        <h3>حضور الفريق</h3>
        <p class="text-muted">سجل الحضور والانصراف</p>
    </a>
    <?php endif; ?>
    <a href="<?= e(url('/employee/dashboard')) ?>" class="card" style="text-decoration:none;color:inherit">
        <h3>لوحتي الشخصية</h3>
